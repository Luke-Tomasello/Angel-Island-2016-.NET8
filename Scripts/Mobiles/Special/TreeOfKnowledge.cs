    ...
    public class TreeOfKnowledge : BaseCreature
    {
        ...
        public override void GenerateLoot()
        {
            if (Core.RuleSets.AngelIslandRules())
            {
                ...
                // 33% chance at best
                Item reward = CreateWeapon((0.32 >= Utility.RandomDouble()) ? 5 : 4);
                PackItem(reward);
            }
            else
            {
        ...
        private abstract class TOKService
        {
            ...
            public virtual bool Validate(TreeOfKnowledge tok, Mobile master, BaseCreature pet, bool message)
            {
                ...
                else if (pet.IsStabled)
                {
                    if (message)
                        ...
                        tok.SayTo(master, "That creature is in your stables. If you wish to see it, talk to your stablemaster.");
            ...
            public bool HandleSpeech(TreeOfKnowledge tok, Mobile from, SpeechEventArgs e)
            {
                ...
                BaseCreature pet = FindPet(petName, from as PlayerMobile) as BaseCreature;
            ...
            public virtual bool HandleGoldGiven(TreeOfKnowledge tok, Mobile from, int given)
            {
                ...
                if (pet.Deleted || ((!pet.Controlled || pet.ControlMaster != from) && !pet.IsStabled))
                {
                    tok.Unregister(from);
                    return false;
                }
