    ...
    public class Item : IPoint3D, IEntity, IHued
    {
		...
		*/
        ...
        private enum SaveFlag
        {
            ...
            LastTouched = 0x40000000,       // who last touched(modified) this item and when
        }
        ...
        public bool Rehydrate()
        {
            ...
            if (faileditem)
            {
                try
                {
                    ...
                    using (FileStream fs = new FileStream(Path.Combine(failedpath, String.Format("{0} {1}.idx", Serial, DateTime.UtcNow.ToString("MM-dd-yyyy HH-mm-ss"))), FileMode.Create, FileAccess.Write))
                    {
                        fs.Write(m_SerializedContentsIdx, 0, m_SerializedContentsIdx.Length);
                        fs.Close();
                    }
                    using (FileStream fs = new FileStream(Path.Combine(failedpath, String.Format("{0} {1}.bin", Serial, DateTime.UtcNow.ToString("MM-dd-yyyy HH-mm-ss"))), FileMode.Create, FileAccess.Write))
                    {
                        fs.Write(m_SerializedContentsBin, 0, m_SerializedContentsBin.Length);
                        fs.Close();
                    }
                    using (StreamWriter sw = new StreamWriter(Path.Combine(failedpath, String.Format("{0} {1}.log", Serial, DateTime.UtcNow.ToString("MM-dd-yyyy HH-mm-ss")))))
                    {
